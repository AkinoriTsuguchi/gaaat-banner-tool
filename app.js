...
function loadTitleFontUsage() {
  try {
    const raw = localStorage.getItem(TITLE_FONT_USAGE_KEY);
    return raw ? JSON.parse(raw) : {};
  } catch (e) {
    return {};
  }
}
function bumpTitleFontUsage(key) {
  try {
    const usage = loadTitleFontUsage();
    usage[key] = (usage[key] || 0) + 1;
    localStorage.setItem(TITLE_FONT_USAGE_KEY, JSON.stringify(usage));
  } catch (e) {
    // localStorage unavailable (private mode, quota, etc.) — usage-based
    // sorting just won't kick in; not worth surfacing an error for this.
  }
}
...
function renderTitleFontPicker() {
  ...
  const usedNotRecommended = keys
    .filter(k => !TITLE_FONT_PRESETS[k].recommended && usage[k] > 0)
    .sort((a, b) => usage[b] - usage[a]);
  const rest = keys.filter(k => !recommended.includes(k) && !usedNotRecommended.includes(k));

  const groups = [
    ['おすすめ', recommended],
    ['よく使うフォント', usedNotRecommended],
    ['その他', rest]
  ];
